...
const { createCheckoutSession, calculateTax, createDiscount }= require("../../lib/helper")
const couponCode = "SOULPIZZA"
const billing = async (req, res, next) => {
    const { id , deliveryType , coupon } = req.body; // Get session ID from client
    ...
    // orderData.tax = totalAmount - totalAmount / ((store.tax + 100) / 100);
          ...
          const session = await createCheckoutSession(lineItems, successUrl, cancelUrl , deliveryCharge , couponData);
